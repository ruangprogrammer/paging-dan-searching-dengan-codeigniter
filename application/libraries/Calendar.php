...
class CI_Calendar {
			...
			for ($i = 0; $i < 7; $i++)
			{
				if ($day > 0 && $day <= $total_days)
				{
					...
					if (isset($data[$day]))
					{
						...
						$out .= str_replace(array('{content}','{title}', '{day}'), array($data[$day]['url'],$data[$day]['note'], $day), $temp);
					}
					else
					{
	...
	public function default_template()
	{
		return array(
			...
			'cal_cell_content'			=> '<a title="{title}" href="{content}">{day}</a>',
			'cal_cell_content_today'	=> '<a title="{title}" href="{content}"><strong>{day}</strong></a>',
